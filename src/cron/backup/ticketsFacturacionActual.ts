...
export const ticketsFacturacionActual = async () => {
  ...
  for (const empresa of empresas) {
    try {
      ...
      const nombre =
        empresa.nombre ?? empresa.get?.("nombre") ?? `#${empresaId}`;
      ...
      const descuentoReclamos =
        Number(
          empresa.get?.("descuento_pendiente_edp") ??
            empresa.descuento_pendiente_edp,
        ) || 0;
      ...
      console.log(`- Descuento por reclamos: $${descuentoReclamos}`);
      console.log(`- Monto facturado final: $${monto_facturado_con_reclamos}`);
      ...
      if (err && (err as any).name === "SequelizeUniqueConstraintError") {
        console.warn(
          `⚠️ [CRON] Omitiendo empresa ${empresa.nombre ?? empresa.id}: El estado de cuenta para este periodo ya fue generado.`,
        );
      } else {
        console.error(
          `❌ [CRON] Error al facturar empresa ${empresa.nombre ?? empresa.id}:`,
          err,
        );
      }
    }
