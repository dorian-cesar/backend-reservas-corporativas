...
export const generateTicketPDFTemplate1 = async (
  ...
  page.drawText(
    "Para subir al bus debe presentar este pasaje impreso o en su celular o no podrá viajar.",
    {
      x: margin,
      y: yPosition,
      size: 10,
  ...
  page.drawText(
    `RUT / Pasaporte: ${ticketData.pasajero.rut || "No disponible"}`,
    {
      x: rightSectionX + 10,
      y: rightFirstY - 55,
      size: 9,
  ...
  page.drawText(
    `Precio Pasaje: CLP$ ${formatNumber(ticketData.ticket.monto_boleto)}`,
    {
      x: rightSectionX + 10,
      y: rightFirstY - 85,
      size: 9,
  ...
  page.drawText(
    "Recuerda presentarte al menos una (1) hora antes de la hora señalada.",
    {
      x: leftSectionX + 25,
      y: secondRowY + 70,
      size: 9,
...
export const generateTicketPDFTemplate2 = async (
  ticketData: TicketPDFData,
): Promise<Uint8Array> => {
  // Crear un nuevo documento PDF con tamaño A4
  const pdfDoc = await PDFDocument.create();
  ...
  const barcodePng = await generateBarcodePng(
    ticketData.ticket.pnrNumber || ticketData.ticket.ticketNumber,
  );
  ...
  page.drawText(
    "Debe acreditar domicilio particular habitual y/o lugar de trabajo en la I o XV Región del país, en control o aduana\n" +
  ...
  page.drawText(
    `${ticketData.ticket.origin} - ${ticketData.ticket.terminal_origen}`,
    {
      x: col2X,
      y: yPosition - 195,
      size: 12,
  ...
  page.drawText(
    `${ticketData.ticket.destination} - ${ticketData.ticket.terminal_destino}`,
    {
      x: col2X,
      y: yPosition - 215,
      size: 12,
  ...
  page.drawText(
    "CONVENIO RESERVAS CORPORATIVAS (CUENTAS CORRIENTES) – NO ANULAR O DEVOLVER\nEN BOLETERÍAS.",
    {
      x: col1X,
      y: yPosition - 35,
      size: 10,
...
export const generateEDPPDF = async (
  ...
  const drawPageHeader = async (page: any, isFirstPage: boolean = false) => {
    ...
    const logoPathPullman = path.resolve(
      __dirname,
      "../assets/logo-pullman-nuevo.png",
    );
    ...
    const logoPathWit = path.resolve(
      __dirname,
      "../assets/logo-wit-dark-full.png",
    );
  ...
  currentPage.drawText(
    `: ${edpData.empresa.cuenta_corriente || "No disponible"}`,
    {
      x: col2X,
      y: yPosition,
      size: 10,
  ...
  const drawTableHeaders = (page: any, posY: number) => {
    ...
    page.drawText("Cantidad de Tickets Confirmados", {
      x: colCantidadX,
      y: posY,
      size: 10,
      font: fontBold,
  ...
  const centrosOrdenados = [...edpData.centros_costo].sort(
    (a, b) => b.monto_facturado - a.monto_facturado,
  );
  ...
  if (tieneCualquierDescuento) {
    ...
    if (
      ...
      currentPage.drawText(
        `-$${formatNumber(edpData.resumen.monto_descuento || 0)}`,
        {
          x: colMontoX,
          y: yPosition,
          size: 9,
